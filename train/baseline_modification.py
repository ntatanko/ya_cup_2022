...
def train_val_split(df, fold, seed = 39):
    df = df.merge(
...
class FeaturesLoader: 
    def __init__(self, features_dir_path, meta_info, device='cpu', test=False, crop_size = 60):
...
class NT_Xent(nn.Module):
    ...
    def forward(self, z_i, z_j):
        ...
        if self.similarity == "cosine":
            ...
            sim = self.similarity_f(z_i.unsqueeze(1), z_j.unsqueeze(0))
...
class SimCLR_infer(nn.Module):
    def __init__(self, encoder, projection_dim):
        super().__init__()
        self.encoder = encoder
        self.n_features = encoder.emb_len
        self.projection_dim = projection_dim
        self.projector = nn.Sequential(
            nn.ReLU(),
            nn.Linear(self.n_features, self.projection_dim, bias=False),
...
#python3 baseline_modification.py --base-dir='/app/_data/artist_data/' --save-dir='ny_1_1' --fold=1 --batch-size=512 --n-chanels=512 --proj-dim=256 --n-epochs=150 --kernel-size=3 --temperature=0.01 --emb-len=1024
#python3 baseline_modification.py --save-dir='ny_1_1' --fold=1 --n-chanels=512 --proj-dim=256 --n-epochs=150 --kernel-size=3 --emb-len=1024
    ...
    BASE_DIR = os.path.join(args.base_dir, 'custom_baseline', args.save_dir)
    ...
    with open(os.path.join(args.base_dir, 'custom_baseline', args.save_dir, 'args.json'), 'w') as f:
        json.dump(args.__dict__, f)

    sim_clr = SimCLR(
        encoder = BasicNet(img_h=512, img_w=60, output_features_size=N_CHANNELS, emb_len=EMB_LEN, kernel_size=KERNEL_SIZE),
